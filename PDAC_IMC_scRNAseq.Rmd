---
  title: "Periductal Fibroblast Density Defines Lymphocyte Exclusion via a CD44-Dependent Stromal Checkpoint in Pancreatic Cancer"
---
...
# Imaging mass cytometry (IMC)-based data transformation and munging for `PDAC_IMC` and `PDAC_sce` objects to generate relevant figures within this manuscript
## all IMC-based datasets for downstream analyses are deposited at doi: 10.5281/zenodo.10582038
...
S2 <- plotSpatial(object = spe[,spe$imageID == "TMA_1_s0_a2_ac.csv"], # can change image ID to plot here 
...
```{r Figure 1E Analysis: Network Analyses}
...
# repeat lines 1091-1113 for each image to generate an "expr" image dataframe
...
mat_P = aggregate(x=expr_k_m_rm0, by=list(rownames(expr_k_m_rm0)), FUN=mean, na.rm=T) # move on to Figure 1E
```
...
##Figure 2
```{r Figure 2C, E-F Analysis: CellSelector}
...
counts
write.csv(counts, file = "image_i_duct_x_cells_subcluster_counts.csv") # open file in Excel and cells can be counted and startified by fibroblast abundance quartile to be plotted externally in GraphPad Prism for Figure 2C,E-F and Supplementary Figure S5-6
...
VlnPlot(NK_roi, features = c("granzyme-B"), ncol = 2, idents = c("Q1", "Q4"), pt.size = 0, dittoColors()[1:3])
```

## Figure 3
```{r fibroblast subclustering}
# The PDAC scRNAseq dataset was previously published at doi: 10.1038/s43018-020-00121-4 
Idents(TotalTissue.combined.harmony_fil) <- 'Expanded_labels' # publicly available scRNAseq dataset 
levels(TotalTissue.combined.harmony_fil)
fibroblasts <- subset(TotalTissue.combined.harmony_fil, idents = c("Fibroblasts"))

DimPlot(fibroblasts, label = T)
DimPlot(fibroblasts, reduction = "umap", group.by = "ID")
DimPlot(fibroblasts, reduction = "umap", group.by = "DiseaseState")
...
fibroblasts <- FindNeighbors(fibroblasts, dims = 1:31)
fibroblasts <- FindClusters(fibroblasts, resolution = seq(from = 0.5, to = 1, by = 0.1))
fibroblasts <- RunUMAP(fibroblasts, dims = 1:31)

head(fibroblasts)
# Clustree res = 0.6
clustree::clustree(fibroblasts, prefix = "RNA_snn_res.")

fibroblasts <- FindNeighbors(fibroblasts, dims = 1:31)
fibroblasts <- FindClusters(fibroblasts, resolution = 0.6)
fibroblasts <- RunUMAP(fibroblasts, dims = 1:31)

p1 <- DimPlot(fibroblasts, reduction = "umap", group.by = "DiseaseState")
p1
p2 <- DimPlot(fibroblasts, reduction = "umap", label = TRUE)
p2
plot_grid(p1, p2)
DimPlot(fibroblasts, reduction = "umap", split.by = "DiseaseState", label = TRUE)
DimPlot(fibroblasts, reduction = "pca", group.by = "DiseaseState")


# Find all fibroblast markers
fibroblasts.all.markers <- FindAllMarkers(object = fibroblasts)
head(fibroblasts.all.markers)

# Heatmap for top 10 markers for each cluster
fibroblasts.all.markers %>% 
  group_by(cluster) %>% 
  dplyr::filter(avg_log2FC > 1) %>%
  slice_head(n = 10) %>%
  ungroup() -> top10
DoHeatmap(fibroblasts, features = top10$gene) + NoLegend()

# Top 10 fibroblast markers
top.10.fibroblast.markers <- as.data.frame(fibroblasts.all.markers %>% group_by(cluster) %>% top_n(n = 10, wt = avg_log2FC))
head(top.10.fibroblast.markers)

# Find markers for every cluster compared to all remaining cells, report only the positive ones
fibroblast.positive.markers <- FindAllMarkers(object = fibroblasts, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
head(fibroblast.positive.markers)

# Top 10 fibroblast positive markers
top.10.fibroblast.positive.markers <- as.data.frame(fibroblast.positive.markers %>% group_by(cluster) %>% top_n(n = 10, wt = avg_log2FC))
head(top.10.fibroblast.positive.markers)

# Heatmap for top 10 positive markers for each cluster
fibroblast.positive.markers %>% 
  group_by(cluster) %>% 
  dplyr::filter(avg_log2FC > 1) %>%
  slice_head(n = 10) %>%
  ungroup() -> top10
...
# Top 10 DE fibroblast markers by disease state
top.10.DE.fibroblast.markers.diseasestate <- as.data.frame(fibroblasts.all.markers %>% group_by("DiseaseState") %>% top_n(n = 10, wt = avg_log2FC))
top.10.DE.fibroblast.markers.diseasestate

# panCAF markers Elyada et al
VlnPlot(fibroblasts, features = c("COL1A1", "FAP", "PDPN", "DCN", "VIM", "DCN", "PDGFRA"))

# iCAF markers Elyada et al, cancer discovery, august 2019
FeaturePlot(fibroblasts, features = c("DCN", "PDGFRA", "CFD", "DPT", "AGTR1", "CXCL12", "IL8","CCL2"))
VlnPlot(fibroblasts, features = c("DCN", "PDGFRA", "CFD", "DPT", "AGTR1", "CXCL12", "IL8","CCL2", "COL14A1"))
RidgePlot(fibroblasts, features = c("DCN", "PDGFRA", "CFD", "DPT", "AGTR1", "CXCL12", "IL8","CCL2"))
DotPlot(fibroblasts, features = c("DCN", "PDGFRA", "CFD", "DPT", "AGTR1", "CXCL12", "IL8","CCL2")) + RotatedAxis()
DoHeatmap(fibroblasts, features = c("DCN", "PDGFRA", "CFD", "DPT", "AGTR1", "CXCL12", "IL8","CCL2"))

# myCAF markers Elyada et al, cancer discovery, august 2019http://127.0.0.1:20413/graphics/plot_zoom_png?width=1458&height=876
FeaturePlot(fibroblasts, features = c("ACTA2", "TAGLN", "FAP", "MMP11", "HOPX", "POSTN", "COL12A1", "THBS2", "IGFBP3", "THY1"))
VlnPlot(fibroblasts, features = c("ACTA2", "TAGLN", "FAP", "MMP11", "HOPX", "POSTN", "COL12A1", "THBS2", "IGFBP3", "THY1"))
RidgePlot(fibroblasts, features = c("ACTA2", "TAGLN", "FAP", "MMP11", "HOPX", "POSTN", "COL12A1"))
DotPlot(fibroblasts, features = c("ACTA2", "TAGLN", "FAP", "MMP11", "HOPX", "POSTN", "COL12A1")) + RotatedAxis()
DoHeatmap(fibroblasts, features = c("ACTA2", "TAGLN", "FAP", "MMP11", "HOPX", "POSTN", "COL12A1"))

# apCAF markers Elyada et al, cancer discovery, august 2019
FeaturePlot(fibroblasts, features = c("HLA-DRA", "HLA-DPA1", "HLA-DQA1"))
VlnPlot(fibroblasts, features = c("CD74","SLPI", "HLA-DRA", "HLA-DPA1", "HLA-DQA1", "XBP1"))
VlnPlot(fibroblasts, features = c("HLA-DRA", "HLA-DPA1", "HLA-DQA1"))

# stellate-like CAF markers (Wang, Y., Liang, Y., Xu, H. et al. Single-cell analysis of pancreatic ductal adenocarcinoma
#identifies a novel fibroblast subtype associated with poor prognosis but better immunotherapy response.
# Cell Discov 7, 36 (2021). https://doi.org/10.1038/s41421-021-00271-4)
VlnPlot(fibroblasts, features = c("MYH11", "RGS5"))

# Assigning cell type identity to clusters
levels(fibroblasts)
new.cluster.ids <- c("myCAF", "myCAF", "Fibroblasts_Normal", "myCAF", "myCAF", "myCAF", "Stellate-like CAF", "iCAF", "myCAF","apCAF")
names(new.cluster.ids) <- levels(fibroblasts)
fibroblasts <- RenameIdents(fibroblasts, new.cluster.ids)
DimPlot(fibroblasts, label = T)

# Filter fibroblasts
fibroblasts_fil <- subset(fibroblasts, idents = c("myCAF", "iCAF", "apCAF", "Stellate-like CAF"))
levels(fibroblasts_fil)
head(fibroblasts_fil)

# Save filtered fibroblasts file
save(fibroblasts_fil, file = "fibroblasts_fil.RData")

# Find all  markers for filtered fibroblasts
fibroblasts_fil_all_markers <- FindAllMarkers(fibroblasts_fil)
head(fibroblasts_fil_all_markers)

# Find all positive markers for filtered fibroblasts
fibroblasts_fil_markers <- FindAllMarkers(fibroblasts_fil, only.pos = T)
head(fibroblasts_fil_markers)

# Top 10 fibroblasts filtered markers
fibroblasts_fil_markers %>% 
  group_by(cluster) %>% 
  dplyr::filter(avg_log2FC > 1) %>%
  slice_head(n = 10) %>%
  ungroup() -> top10
DoHeatmap(fibroblasts_fil, features = top10$gene) + NoLegend()

DimPlot(fibroblasts_fil, label = T, split.by = "DiseaseState")
DimPlot(fibroblasts_fil, label = T, group.by = "DiseaseState")
DimPlot(fibroblasts_fil, reduction = "pca", group.by = "DiseaseState")

# Find differentially expressed fibroblast markers
#myCAF v iCAF
fibroblasts_fil_myCAF_iCAF_de_markers <- FindMarkers(fibroblasts_fil, ident.1 = "myCAF", ident.2 = "iCAF", test.use = "MAST", only.pos = TRUE)
head(fibroblasts_fil_myCAF_iCAF_de_markers)

# Visualize marker genes in identified fibroblast subclusters
pan.caf.markers <- c("COL1A1", "FAP", "PDPN", "DCN", "VIM")
VlnPlot(fibroblasts_fil, features = pan.caf.markers)
h1 <- DoHeatmap(fibroblasts_fil, features = pan.caf.markers) + ylab("Pan-CAF Markers")
h2 <- DoHeatmap(fibroblasts_fil, features = pan.caf.markers, group.by = "DiseaseState") + ylab("Pan-CAF Markers")
plot_grid(h1, h2)

icaf.markers <- c("CFD", "DPT", "AGTR1", "CXCL12", "IL8","CCL2")
VlnPlot(fibroblasts_fil, features = icaf.markers)
h1.2 <- DoHeatmap(fibroblasts_fil, features = icaf.markers) + ylab("iCAF Markers")
h2.2 <- DoHeatmap(fibroblasts_fil, features = icaf.markers, group.by = "DiseaseState") + ylab("iCAF Markers")
plot_grid(h1.2, h2.2)

mycaf.markers <- c("ACTA2", "TAGLN", "FAP", "MMP11", "HOPX", "POSTN", "COL12A1", "THBS2", "IGFBP3", "THY1")
VlnPlot(fibroblasts_fil, features = mycaf.markers)

apcaf.markers <- c("HLA-DRA", "HLA-DPA1", "HLA-DQA1")
VlnPlot(fibroblasts_fil, features = apcaf.markers)

stellate.likecaf.markers <- c("MYH11", "RGS5")
VlnPlot(fibroblasts_fil, features = stellate.likecaf.markers)
```

```{r CytoTRACE analysis of epithelial cells}
...
epi.integrated <- FindClusters(epi.integrated, resolution = seq(from = 0.7, to = 0.75, by = 0.01))
...
```{r integrating CytoTRACE and fibroblast annotations to Steele et al. dataset}
obj <- TotalTissue.combined.harmony_fil

obj$subclusters <- as.factor(obj$Expanded_labels) # copy metadata slot of original annotations that will be changed to include all new subcluster information

to_add <- as.data.frame(epi.integrated$cutoff_0.72_labels)
to_add_2 <- as.data.frame(Idents(fibroblasts_fil))

obj <- AddMetaData(obj, metadata = to_add, col.name = "epi.cytoTRACE_0.72")
obj <- AddMetaData(obj, metadata = to_add_2, col.name = "fibroblast_subclusters")

obj$epi.cytoTRACE_0.72 <- as.factor(obj@meta.data[["epi.cytoTRACE_0.72"]])
obj$firoblast_subclusters <- as.factor(obj@meta.data[["fibroblast_subclusters"]])

y <- (as.data.frame(obj$epi.cytoTRACE_0.72))
x <- (as.data.frame(obj$subclusters))
a <- (as.data.frame(obj$fibroblast_subclusters))
z <- row.names(x) == row.names(y)
zz <- row.names(x) == row.names(a)
zzz <- row.names(y) == row.names(a)
...
xya <- cbind(x,y,a)
str(xya) # check that columns are characters in order to remove NAs
xya$`obj$subclusters` <- as.character(xya$`obj$subclusters`)
xya$`obj$epi.cytoTRACE_0.72` <- as.character(xya$`obj$epi.cytoTRACE_0.72`)
xya$`obj$fibroblast_subclusters` <- as.character(xya$`obj$fibroblast_subclusters`)

xya$`obj$epi.cytoTRACE_0.72`[is.na(xya$`obj$epi.cytoTRACE_0.72`)] <- xya$`obj$subclusters`[is.na(xya$`obj$epi.cytoTRACE_0.72`)]
xya$`obj$fibroblast_subclusters`[is.na(xya$`obj$fibroblast_subclusters`)] <- xya$`obj$epi.cytoTRACE_0.72`[is.na(xya$`obj$fibroblast_subclusters`)]

xya$subclusters <- xya$`fibroblast_subclusters`

xya$subclusters <- as.factor(xya$`obj$fibroblast_subclusters`)

xya$subclusters <- recode_factor(xya$subclusters,
                              positive = "Epithelial_Cancer",
                              negative = "Epithelial_Normal")
#create a dataframe containing all subcluster information to add to seurat object metadata
xya_rm <- subset(xya, select = -c(1,2,3)) # removing first 3 column so not accidentally overwriting original annotations from that original slot

obj <- AddMetaData(obj, metadata = xya_rm)

plot(getSankey(obj$Expanded_labels, obj$subclusters)) # check that idents are aligned
```
...
# subset original object to assess L-R interactions between NK cell, epithelial cell, and fibroblast subpopulations
Idents(obj) <- obj$subclusters
levels(Idents(obj))
obj_NK_epi_fib <- subset(obj, idents = c("Epithelial_Cancer",
                                         "Epithelial_Normal",
                                         "apCAF",
                                         "Fibroblasts_Normal",
                                         "iCAF",
                                         ...
                                         "NK Cells",
                                         "Stellate-like CAF"))
obj_all_subclusters <- obj
obj <- obj_NK_epi_fib

Idents(obj) <- obj$DiseaseState
AdjNorm <- subset(obj, idents = "AdjNorm")
Idents(AdjNorm) <- AdjNorm$subclusters
levels(AdjNorm$subclusters)
cellchat_AdjNorm <- createCellChat(AdjNorm, group.by = "subclusters", assay = "RNA") # create CellChat object
levels(cellchat_AdjNorm@idents) # check that there are no missing idents and drop any unused levels
AdjNorm$subclusters <- ordered(AdjNorm$subclusters, levels = c("NK Cells",
                                                         "Epithelial_Cancer",
                                                         "Epithelial_Normal",
                                                         "Stellate-like CAF",
                                                         "iCAF",
...
cellchat_AdjNorm <- CellChat::createCellChat(AdjNorm, group.by = "subclusters", assay = "RNA") # create CellChat object
cellchat_AdjNorm@idents
unique(cellchat_AdjNorm@idents)
levels(cellchat_AdjNorm@idents)
...
# set the used CellChat database in the object
...
#check that there are no unused levels
levels(cellchat@idents)
cellchat@idents <- droplevels(cellchat@idents, c("Acinar",
                                                 "B Cells",
                                                 "CD4 T Cells",
                                                 "CD8 T Cells",
                                                 "Dendritic Cell",
...
df.net_LR_AdjNorm <- CellChat::subsetCommunication(cellchat) 
df.net_SIG_AdjNorm <- CellChat::subsetCommunication(cellchat, slot.name = "netP") 
...
Idents(PDAC) <- PDAC$subclusters
levels(PDAC$subclusters)

PDAC$subclusters <- ordered(PDAC$subclusters, levels = c("NK Cells",
                                                         "Epithelial_Cancer",
                                                         "Epithelial_Normal",
                                                         "Stellate-like CAF",
                                                         "iCAF",
...
cellchat_PDAC <- CellChat::createCellChat(PDAC, group.by = "subclusters", assay = "RNA") # create CellChat object
...
# Extract the inferred cellular communication network as a data frame
df.net_LR_PDAC <- subsetCommunication(cellchat) 
df.net_SIG_PDAC <- subsetCommunication(cellchat, slot.name = "netP") 
...
cellchat_AdjNorm <- liftCellChat(cellchat_AdjNorm, group.new)
cellchat_PDAC <- liftCellChat(cellchat_PDAC, group.new)

object.list <- list(AdjNorm = cellchat_AdjNorm,
                    PDAC = cellchat_PDAC)

cellchat <- mergeCellChat(object.list, add.names = names(object.list), merge.data = T)

# liftCellChat() and mergeCellChat() functions scramble the names of the cell identies in the cellchat@idents$joint factor. Renaming the cell names based on the Seurat object they were derived from
cc_meta <- as.data.frame(cellchat@idents$joint)
cc_meta$rownames <- rownames(obj@meta.data)
cc_meta$subclusters <- obj@meta.data$subclusters
rownames(cc_meta) <- cc_meta$rownames
cc_meta <- cc_meta[,2:3]
str(cc_meta) # confirm subclusters are factors
first_factor <- cc_meta$subclusters
first_factor <- droplevels(first_factor, c("Acinar",
                                           "B Cells",
                                           "CD4 T Cells",
                                           "CD8 T Cells",
                                           "Dendritic Cell",
                                           ...
                                           "Plasma Cells",
                                           "T Reg"))
names(first_factor) <- rownames(cc_meta)
cellchat@idents$joint <- first_factor

levels(cellchat@idents$PDAC)
levels(cellchat@idents$AdjNorm)
levels(cellchat@idents$joint)

cellchat@idents$joint <- ordered(cellchat@idents$joint, levels = c("NK Cells",
                                                         "Epithelial_Cancer",
                                                         "Epithelial_Normal",
                                                         "Stellate-like CAF",
                                                         "iCAF",
...
##confirmatory Sankey plots to show that cell identities align
plot(getSankey(PDAC$subclusters, cellchat@idents$PDAC))
plot(getSankey(AdjNorm$subclusters, cellchat@idents$AdjNorm))
plot(getSankey(obj$subclusters, cellchat@idents$joint))
...
net.up <- subsetCommunication(cellchat, net = net, datasets = "PDAC", ligand.logFC = 0.2, receptor.logFC = 0.2)
...
View(x)
x$DiseaseState <- c("PDAC", "PDAC", "PDAC", "PDAC", "PDAC", "PDAC", "PDAC", "PDAC",
                    "AdjNorm", "AdjNorm", "AdjNorm", "AdjNorm", "AdjNorm", "AdjNorm", "AdjNorm")

x$labels <- ordered(x$labels, levels = c("NK Cells",
                                         "Epithelial_Cancer",
                                         "Epithelial_Normal",
                                         "Stellate-like CAF",
                                         "iCAF",
...
color.use <- scPalette(length(levels(cellchat@idents$joint)))
color.use }
    ...
    row.show = NULL, col.show = NULL) 
{
    ...
    net <- net.diff
    if ((!is.null(targets.use)) | (!is.null(sources.use))) {
    ...
    if (is.null(color.use)) {
        color.use <- scPalette(length(1:(ncol(mat))))
        names(color.use) <- NULL
    }
    ...
    ht1 = Heatmap(mat, col = color.heatmap.use, na_col = "white", 
        ...
        column_names_gp = gpar(fontsize = font.size), column_title_gp = gpar(fontsize = font.size.title), 
        column_names_rot = 90, column_title = "Sources (Sender)", 
        row_title = "Targets (Receiver)", row_title_gp = gpar(fontsize = font.size.title), 
...
netVisual_heatmap_edit(cellchat, measure = "weight")
```

```{r Figure 3C}
levels(cellchat@idents$joint) # identify levels for cell types to choose approriate sources and targets
rankNet(cellchat, mode = "comparison", stacked = F, do.stat = TRUE, sources.use = c(2,4,5,7,8), targets.use = c(1))
```

```{r Figure 3D}
netVisual_bubble(cellchat, sources.use = c(2,4,5,7,8), targets.use = c(1), comparison = c(1, 2),  angle.x = 45, remove.isolate = T, signaling = c("COLLAGEN"))
```

# Supplementary Figures
## Supplementary Figure S1
```{r Supplementary Figure S1B}
...
```{r Supplementary Figure S1C}
obj <- PDAC
dat <- as.matrix(GetAssayData(obj, assay = "RNA", layer = "data"))

# prep for single-cell heatmap
set.seed(123)
cn <- colnames(dat)
keep_names <- sample(cn, 15000) # sub-sample 15k cells
dat_subset <- dat[c("pankeratin",
                     "Ecadherin",
                     "FAP",
                     "aSMA",
                     "vimentin",
...
meta_sub <- obj@meta.data[colnames(dat_subset), c("cellType", "TMA")]
meta_sub$cellType <- as.factor(meta_sub$cellType)

# reorder cell types...
old_lvls <- levels(meta_sub$cellType)
last     <- old_lvls[length(old_lvls)]
rest     <- old_lvls[-length(old_lvls)]
new_lvls <- append(rest, last, after = 1)
meta_sub$cellType <- factor(meta_sub$cellType, levels = new_lvls)
meta_sub$TMA <- as.factor(meta_sub$TMA)

# prepare colors
celltype_levels <- levels(meta_sub$cellType)
origid_levels   <- levels(meta_sub$TMA)
celltype_colors <- setNames(
  brewer.pal(n = length(celltype_levels), "Dark2"),
  celltype_levels
)

base_pal <- brewer.pal(min(12, n_levels), "Set3")

# Use colorRampPalette to interpolate up to n_levels colors
origid_colors <- setNames(
  colorRampPalette(base_pal)(n_levels),
  origid_levels
)

# order by celltype
ord <- order(meta_sub$cellType)
mat_ord     <- dat_subset[, ord]
meta_sub_ord <- meta_sub[ord, , drop = FALSE]

# ordered annotation
ha_ord <- HeatmapAnnotation(
  df = meta_sub_ord,
  col = list(
    cellType  = celltype_colors,
    orig.ident = origid_colors
  )
)

# scale rows, i.e. ab specific z-scores
mat_ord <- t(scale(t(mat_ord)))

# adjust color scaling
library(scales)
col_fun_tight <- colorRamp2(
  breaks = c(-2, 0, +2),
  colors = c(muted("blue"), "lightgray", "red")
)
...
mat <- subcluster_mean@assays@data@listData[["logcounts"]][sort(rownames(subcluster_mean@assays@data@listData[["logcounts"]])), sort(colnames(subcluster_mean@assays@data@listData[["logcounts"]]))]

pheatmap(mat, cluster_cols = F, cluster_rows = F, scale = "row", color = colorRampPalette(c("#053061", "#F7F7F7", "#67001F"))(50))
```

```{r Supplementary Figure S2B}
dittoBarPlot(PDAC,
             "subcluster",
             group.by = "Case_Number")
```
...
SF4 <- colData(sce) %>% as_tibble() %>%
  group_by(cn_subcluster, subcluster) %>%
  dplyr::summarize(count = n(), .groups = "drop")

write.csv(SF4, file = "SF3_df.csv") # open file to graph data in GraphPad Prism
```

## Supplementary Figure S5-S6
```{r Supplementary Figure S5-S6}
# refer to Figure 2C, E-F Analysis: CellSelector
```
...
NK_roi <- subset(PDAC, idents = "NK cell")
Idents(NK_roi) <- NK_roi$ROI_Quar
VlnPlot(NK_roi, features = c("CD69", "CD107a", "IFNg"), ncol = 2, idents = c("Q1", "Q4"), pt.size = 0, dittoColors()[1:3])
```
## Supplementary Figure S8
```{r Supplementary Figure S8A}
Idents(obj) <- obj$Expanded_labels
length(unique(obj$Expanded_labels))
dim_plot <- DimPlot(obj, cols = dittoColors()[1:16], ncol = 2)
dim_plot$data$ident <- factor(x = dim_plot$data$ident, levels = c("Acinar", "B Cells", "CD4 T Cells", "CD8 T Cells", "Dendritic Cell", "Endocrine", "Endothelial", "Epithelial", "Fibroblasts", "Granulocyte", "Macrophage", "Mast Cells", "NK Cells","Pericytes", "Plasma Cells", "T Reg"))
dim_plot
```
...
hist(epi.integrated$results.CytoTRACE,100,F, xlab = "CytoTRACE Score")  # generates histogram of CytoTRACE scores from integrated epithelial data
lines(mixmodel,lwd=1.5,col="red")
abline(v=msEst$parameters$pro,lty=2,col="blue")
```

```{r Supplementary Figure S8C}
CAF_features <- c("COL1A1",   
                  "FAP",  
                  "PDPN",
                  "DCN",
                  "VIM",
...
CAF_features_mean <- aggregateAcrossCells(as(sce, "SingleCellExperiment"),  
                                      ids = sce$subclusters, 
                                      statistics = "mean",
                                      use.assay.type = "logcounts", 
                                      subset.row = CAF_features)

mat <- CAF_features_mean@assays@data@listData[["logcounts"]][sort(rownames(CAF_features_mean@assays@data@listData[["logcounts"]])), sort(colnames(CAF_features_mean@assays@data@listData[["logcounts"]]))]
mat <- mat[,c(2,11,13,16,20)]
mat <- as.matrix(mat)
pheatmap(mat, cluster_cols = F, cluster_rows = F, scale = "row", color = colorRampPalette(c("#053061", "#F7F7F7", "#67001F"))(50))

```

```{r Supplementary Figure S8D}
Idents(obj) <- obj$subclusters

dim_plot <- DimPlot(obj, reduction.use = "umap", cols = dittoColors()[1:21])

dim_plot$data$ident <- factor(x = dim_plot$data$ident, levels = c("Acinar", "B Cells", "CD4 T Cells", "CD8 T Cells", "T Reg", "Dendritic Cell", "Endocrine", "Endothelial", "Epithelial_Cancer", "Epithelial_Normal", "apCAF", "iCAF", "myCAF", "Fibroblasts_Normal", "Stellate-like CAF", "Mast Cells", "Myeloid", "NK Cells", "Pericytes", "Plasma Cells")) # change the order of the factor levels

dim_plot
```

```{r Supplementary Figure S8E}
dittoBarPlot(obj, var = "subclusters", group.by = "ID", split.by = "DiseaseState")
```
...
deg_sl_fib <- FindMarkers(obj, 
                       ident.1 = "PDAC", 
                       ident.2 = "AdjNorm",
                       group.by = "DiseaseState", 
                       verbose = TRUE, 
                       subset.ident = "Stellate-like CAF", 
                       features = features)

deg_iCAF <- FindMarkers(obj, 
                       ident.1 = "PDAC", 
                       ident.2 = "AdjNorm",
                       group.by = "DiseaseState", 
                       verbose = TRUE, 
